...
public class BF_PlayerSnow : MonoBehaviour
{
    ...
    private void CheckIceCols(float snowCol)
    {
        ...
        if (snowCol == -1)
        {
            if (playerCollider.sharedMaterial != playerMatDefault)
            {
                playerCollider.sharedMaterial = playerMatDefault;
            }
            return;
        }


        if (lerpIce <= 0.925f && playerCollider.sharedMaterial != playerMatIce)
        {
            playerCollider.sharedMaterial = playerMatIce;
            rB.angularDrag = 0.25f;
        }
        else if(lerpIce >= 0.925f && playerCollider.sharedMaterial != playerMatSnow)
        {
            playerCollider.sharedMaterial = playerMatSnow;
            rB.angularDrag = 5f;
        }
    }
